    ...
    for (let k = 0; k < taille; k++) {
      if (monkeys[j].operation === "+") {
        ...
        if (monkeys[j].nombre === "old") {
          monkeys[j].objects[0] = monkeys[j].objects[0] * monkeys[j].objects[0];
        } else {
          monkeys[j].objects[0] =
            monkeys[j].objects[0] * parseInt(monkeys[j].nombre);
        }
      }
      monkeys[j].objects[0] = Math.trunc(monkeys[j].objects[0] / 3);
      if (monkeys[j].objects[0] % monkeys[j].test === 0) {
        monkeys[monkeys[j].verifT].objects.push(monkeys[j].objects[0]);
        monkeys[j].objects.shift();
      } else {
        monkeys[monkeys[j].verifF].objects.push(monkeys[j].objects[0]);
        monkeys[j].objects.shift();
      }
      compteur[j]++;
    }
